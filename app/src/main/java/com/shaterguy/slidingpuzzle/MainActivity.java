...
public class MainActivity extends Activity {
 ...
 private void receive(Intent intent){
  ...
  Uri uri=Build.VERSION.SDK_INT>=33?intent.getParcelableExtra(Intent.EXTRA_STREAM,Uri.class):intent.getParcelableExtra(Intent.EXTRA_STREAM);
 ...
 @Override protected void onPause(){pauseClock();foreground=false;if(confetti!=null)confetti.stop();save();super.onPause();}
 ...
 void move(int index){
  ...
  save();updateStats();
  board.animateMove(index,blank,()->{if(isDestroyed())return;render();if(completed)celebrate();});
 }
